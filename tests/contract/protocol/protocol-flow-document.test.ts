...
test("Flow graph rejects unsupported node types and malformed edges", (): void => {
	assert.equal(clientRequestSchema.safeParse({ type: "request", id: "request-1", method: "flow.node.create", params: { flowId: "flow-a", revision: 1, type: "tool", x: 0, y: 0 } }).success, false);
