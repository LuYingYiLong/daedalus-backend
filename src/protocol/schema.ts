...
const providerModelRoutingSchema = z
	.object({
		...
		videoGeneration: providerTaskModelRefSchema.nullable().optional(),
...
// Flow node types are namespaced so trusted plugins can add definitions without
// extending the wire protocol. Built-ins use the `builtin/*` namespace.
export const flowNodeTypeIdSchema = z.string().trim().regex(
...
export const flowMediaArtifactRefSchema = z.object({
	...
	previewArtifactId: flowIdentifierSchema.optional(),
...
const flowOutputConfigSchema = z.object({ format: z.enum(["text", "json", "image", "video", "auto"]).default("text") }).strict();
const flowNoteConfigSchema = z.object({ text: z.string().max(200_000).default("") }).strict();
const flowImageGenerationConfigSchema = z.object({
	provider: z.string().max(120).default(""),
	model: z.string().max(240).default(""),
	prompt: z.string().max(200_000).default(""),
	negativePrompt: z.string().max(200_000).default(""),
	aspectRatio: z.string().max(32).default("1:1"),
...
const flowMediaOutputConfigSchema = z.object({ format: z.enum(["preview", "download", "workspace"]).default("preview") }).strict();
export const flowDocumentNodeConfigSchemas = {
	...
	"builtin/output": flowOutputConfigSchema,
	"builtin/note": flowNoteConfigSchema,
	"builtin/text-to-image": flowImageGenerationConfigSchema,
	"builtin/image-to-image": flowImageGenerationConfigSchema,
	"builtin/text-to-video": flowVideoGenerationConfigSchema,
	"builtin/image-to-video": flowVideoGenerationConfigSchema,
	"builtin/media-output": flowMediaOutputConfigSchema,
} as const;
...
export const flowNodeTypeDefinitionSchema = z.object({
	...
	pluginFingerprint: z.string().trim().min(1).max(256),
	...
	summaryFields: z.array(z.string().min(1).max(120)).max(16),
	ui: z.discriminatedUnion("kind", [
...
export const flowDocumentNodeRunSchema = z.object({
	...
	configVersion: z.number().int().positive(),
	status: flowDocumentNodeStatusSchema,
	providerJobId: z.string().trim().min(1).max(240).nullable().optional(),
...
export const flowOperationSchema = z.discriminatedUnion("kind", [
	z.object({
		...
		kind: z.literal("node.create"),
		baseGraphRevision: z.number().int().positive().optional(),
		payload: z.object({ nodeId: flowIdentifierSchema, typeId: flowNodeTypeIdSchema, title: z.string().trim().min(1).max(200).optional(), x: z.number().finite(), y: z.number().finite(), config: flowDocumentNodeConfigSchema.optional() }).strict(),
	}).strict(),
	z.object({
		mutationId: flowIdentifierSchema,
		kind: z.literal("node.update"),
		baseGraphRevision: z.number().int().positive().optional(),
		payload: z.object({ nodeId: flowIdentifierSchema, title: z.string().trim().min(1).max(200).optional(), config: flowDocumentNodeConfigSchema.optional() }).strict(),
	}).strict(),
	...
	z.object({
		...
		baseGraphRevision: z.number().int().positive().optional(),
		payload: z.object({ edgeId: flowIdentifierSchema, sourceNodeId: flowIdentifierSchema, sourcePort: flowIdentifierSchema, targetNodeId: flowIdentifierSchema, targetPort: flowIdentifierSchema, dataType: flowDocumentPortTypeSchema }).strict(),
	}).strict(),
	z.object({ mutationId: flowIdentifierSchema, kind: z.literal("edge.delete"), baseGraphRevision: z.number().int().positive().optional(), payload: z.object({ edgeId: flowIdentifierSchema }).strict() }).strict(),
	z.object({ mutationId: flowIdentifierSchema, kind: z.literal("viewport.update"), baseLayoutRevision: z.number().int().positive().optional(), payload: z.object({ x: z.number().finite(), y: z.number().finite(), zoom: z.number().finite().positive() }).strict() }).strict(),
]);
export const flowPatchAckSchema = z.object({
	flowId: flowIdentifierSchema,
	graphRevision: z.number().int().positive(),
	layoutRevision: z.number().int().positive(),
	acceptedMutationIds: z.array(flowIdentifierSchema),
	operations: z.array(flowOperationSchema),
}).strict();
export const flowPatchAppliedEventDataSchema = flowPatchAckSchema.extend({ clientId: flowIdentifierSchema }).strict();
...
export const clientRequestSchema = z.discriminatedUnion("method", [
	...
	z.object({
		type: z.literal("request"),
		id: z.string(),
		method: z.literal("computer.tool.result"),
	...
	z.object({
		type: z.literal("request"),
		id: z.string(),
		method: z.literal("flow.patch.commit"),
		params: z.object({
			flowId: flowIdentifierSchema,
	...
	z.object({ type: z.literal("request"), id: z.string(), method: z.literal("plugin.update.operation.cancel"), params: z.object({ operationId: z.string().min(1).max(240) }).strict() }),
	z.object({ type: z.literal("request"), id: z.string(), method: z.literal("plugin.development.runs.list"), params: z.object({ pluginId: z.string().min(1).max(240).optional(), limit: z.number().int().min(1).max(100).optional() }).strict().optional() }),
	z.object({ type: z.literal("request"), id: z.string(), method: z.literal("plugin.development.runs.get"), params: z.object({ runId: z.string().min(1).max(240) }).strict() }),
	z.object({ type: z.literal("request"), id: z.string(), method: z.literal("plugin.changelog.generate"), params: z.object({ pluginId: z.string().min(1).max(240), nextVersion: z.string().min(1).max(64), aiText: z.string().max(128000).optional() }).strict() }),
	z.object({ type: z.literal("request"), id: z.string(), method: z.literal("plugin.changelog.apply"), params: z.object({ draftId: z.string().min(1).max(240), expectedRevision: z.string().length(64), accepted: z.literal(true), editedText: z.string().max(128000).optional() }).strict() }),
	z.object({ type: z.literal("request"), id: z.string(), method: z.literal("plugin.release.preview"), params: z.object({ pluginId: z.string().min(1).max(240), nextVersion: z.string().min(1).max(64), aiText: z.string().max(128000).optional() }).strict() }),
	z.object({ type: z.literal("request"), id: z.string(), method: z.literal("plugin.release.confirm"), params: z.object({ draftId: z.string().min(1).max(240), expectedRevision: z.string().length(64), editedText: z.string().max(128000).optional() }).strict() }),
	z.object({ type: z.literal("request"), id: z.string(), method: z.literal("plugin.release.export"), params: z.object({ draftId: z.string().min(1).max(240), expectedRevision: z.string().length(64), editedText: z.string().max(128000).optional() }).strict() }),
	z.object({ type: z.literal("request"), id: z.string(), method: z.literal("plugin.publish.confirm"), params: z.object({ artifactPath: z.string().min(1).max(4096), registry: z.string().url().max(2048) }).strict() }),
	z.object({
		type: z.literal("request"),
		id: z.string(),
