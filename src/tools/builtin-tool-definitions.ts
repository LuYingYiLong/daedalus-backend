...
function supportsSourceFolderParameter(name: string): boolean {
	return name.startsWith("mcp_workspace_")
		...
		|| name === "mcp_video_import_flow_videos"
...
const IMAGE_GENERATION_TOOL_DEFINITIONS: ChatCompletionTool[] = [
	...
	createSceneToolDefinition("mcp_video_import_flow_videos", "Save Flow video artifacts to new workspace files. Only available with an active Flow context; never overwrites files.", { flowId: { type: "string" }, saveId: { type: "string" }, items: { type: "array", maxItems: 100, items: { type: "object", properties: { itemKey: { type: "string" }, artifactId: { type: "string" }, relativePath: { type: "string" } }, required: ["artifactId", "relativePath"], additionalProperties: false } } }, ["flowId", "saveId", "items"]),
	createSceneToolDefinition(
