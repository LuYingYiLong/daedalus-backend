...
export function describeToolEvent(toolName: string, args: Record<string, unknown>, workspaceId?: string | undefined): ToolEventDisplay {
	...
	if (toolName.startsWith("mcp_computer_")) {
		...
		const title = toolName === "mcp_computer_action" ? "Control authorized window" : toolName === "mcp_computer_request_access" ? (args.mode === "control" ? "Request control window" : "Request observation window") : toolName === "mcp_computer_observe" ? "Read window structure and text" : toolName === "mcp_computer_locate" ? "Locate icon in authorized window" : "View observation screenshot";
	...
	if (toolName === "mcp_video_import_flow_videos") return createDisplay("image", "Flow", "write", "Save videos", `Save ${Array.isArray(args.items) ? args.items.length : 0} video(s) to the workspace`, { kind: "file", label: "Flow videos" });
