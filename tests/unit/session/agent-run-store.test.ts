...
test("cancelled run preserves the stored user request for unfinished-turn context", async (): Promise<void> => {
	const directory: string = await mkdtemp(join(tmpdir(), "daedalus-agent-run-cancelled-context-"));
	const databasePath: string = join(directory, "sessions.sqlite3");
	const { resetSessionDatabaseForTests } = await import("../../../src/session/session-database.js");
	await resetSessionDatabaseForTests(databasePath);
	try {
		const { createSession, openSession, saveSession } = await import("../../../src/session/session-store.js");
		const metadata = await createSession("Cancelled context", undefined);
		await saveSession(metadata.id, [{
			role: "user",
			content: "make the map infinite",
		...
		await saveAgentRunState(cancelled);

		const opened = await openSession(metadata.id);
		assert.equal(opened.messages[0]?.excludeFromLlmContext, undefined);
		assert.equal(opened.messages[0]?.content, "make the map infinite");
	} finally {
		await resetSessionDatabaseForTests();
		await rm(directory, { recursive: true, force: true });
	}
