...
export const flowImageSaveSchema = z.object({
...
export async function saveFlowImages(args: unknown, workspaceId: string | undefined, sessionId: string | undefined, signal?: AbortSignal): Promise<Record<string, unknown>> {
	const request = flowImageSaveSchema.parse(args);
	if (sessionId !== `flow:${request.flowId}` || !workspaceId) throw new Error("flow_image_save_scope_invalid");
	...
	if (!workspace || flow?.workspace_id !== workspaceId) throw new Error("flow_image_save_workspace_invalid");
	...
	for (const [index, item] of request.items.entries()) {
		...
		if (artifact.ref.flowId !== request.flowId || !["image/png", "image/jpeg", "image/webp"].includes(artifact.ref.mimeType)) throw new Error("flow_image_save_artifact_invalid");
		const expectedExt = artifact.ref.mimeType === "image/jpeg" ? ".jpg" : `.${artifact.ref.mimeType.split("/")[1]}`;
		if (path.extname(item.relativePath).toLowerCase() !== expectedExt) throw new Error("flow_image_save_extension_invalid");
		const destination = await resolveSafeDestination(workspace.rootPath, item.relativePath);
		const previous = db.prepare("SELECT relative_path,sha256 FROM flow_image_saves WHERE save_id=? AND item_index=?").get(key,itemIndex) as { relative_path: string; sha256: string } | undefined;
		...
		if (previous) {
			...
			try {
				...
				if (info.isSymbolicLink() || !info.isFile()) throw new Error("flow_image_save_link_invalid");
		...
		if (!saved) for (let suffix = 0; suffix < 10000; suffix++) {
			...
			// 先落盘写入意图；崩溃恢复使用内容哈希确认，不再次生成文件
			db.prepare("INSERT INTO flow_image_saves(save_id,item_index,flow_id,relative_path,sha256) VALUES(?,?,?,?,?) ON CONFLICT(save_id,item_index) DO UPDATE SET relative_path=excluded.relative_path,sha256=excluded.sha256").run(key,itemIndex,request.flowId,relativePath,artifact.ref.sha256);
			try { await writeFile(target.absolutePath, artifact.bytes, { flag: "wx" }); saved = true; break; }
			catch (error) { if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error; }
		}
		if (!saved) throw new Error("flow_image_save_name_limit");
