...
test("schema 10 upgrades to current schema without changing observation or screenshot, and reopens idempotently", async () => {
  await withStore(async (db, sessionId) => {
	...
	assert.equal(migrated.prepare("PRAGMA user_version").get()!.user_version, 22);
	assert.equal(migrated.prepare("SELECT count(*) AS n FROM schema_migrations WHERE version IN (10,22)").get()!.n, 2);
	...
	assert.equal(reopened.prepare("SELECT count(*) AS n FROM schema_migrations WHERE version=22").get()!.n, 1);
