...
test("schema 10 upgrades to current schema without changing observation or screenshot, and reopens idempotently", async () => {
  await withStore(async (db, sessionId) => {
    await saveComputerScreenshot(sessionId, "turn-1", { ...observation(), dataUrl: PNG });
    const before = readRow(db, sessionId);
    ...
    assert.equal(migrated.prepare("PRAGMA user_version").get()!.user_version, 27);
    assert.equal(migrated.prepare("SELECT count(*) AS n FROM schema_migrations WHERE version IN (10,27)").get()!.n, 2);
    ...
    assert.equal(reopened.prepare("SELECT count(*) AS n FROM schema_migrations WHERE version=27").get()!.n, 1);
