...
export function registerComposableDefinitions(register: typeof registerBuiltin): void {
	...
	register("image-input", "media", "Image Input", { path: "" }, [fixed("path")], [output("image", ["image"]), output("size", ["size"])], z.object({ path: z.string().max(4000) }).strict(), { workspaceRequired: true, cachePolicy: "never", fieldControls: { path: "workspace-file" } });
	const image = input("image", ["image"], "one-or-many");
	const result = [output("images", ["image"], "many")];
	register("image-resize", "media", "Image Resize", { width: 1024, height: 1024, fit: "contain", background: { r: 0, g: 0, b: 0, a: 0 } }, [image, { ...input("size", ["size"]), required: false, defaultConnect: false }, hybrid("width", "number"), hybrid("height", "number"), fixed("fit"), hybrid("background", "color")], result, z.object({ width: positive, height: positive, fit: z.enum(["contain", "cover", "fill"]), background: flowColorSchema }).strict(), { fieldControls: { background: "color" } });
	register("image-crop", "media", "Image Crop", { x: 0, y: 0, width: 512, height: 512 }, [image, ...["x", "y", "width", "height"].map(id => hybrid(id, "number"))], result, z.object({ x: coord, y: coord, width: positive, height: positive }).strict());
	register("image-rotate", "media", "Image Rotate", { angle: 0, flip: false, flop: false }, [image, hybrid("angle", "number"), hybrid("flip", "boolean"), hybrid("flop", "boolean")], result, z.object({ angle: z.number().min(-360).max(360), flip: z.boolean(), flop: z.boolean() }).strict());
	register("image-composite", "media", "Image Composite", { x: 0, y: 0, opacity: 1 }, [image, input("overlay", ["image"]), hybrid("x", "number"), hybrid("y", "number"), hybrid("opacity", "number")], result, z.object({ x: coord, y: coord, opacity: z.number().min(0).max(1) }).strict());
	register("image-convert", "media", "Image Convert", { format: "png", quality: 90 }, [image, fixed("format"), fixed("quality")], result, z.object({ format: z.enum(["png", "jpeg", "webp"]), quality: z.number().int().min(1).max(100) }).strict());
	register("save-images", "workspace", "Save Images", { directory: "outputs", fileName: "{runId}-{index}" }, [input("images", ["image"], "one-or-many"), fixed("directory"), fixed("fileName")], [], z.object({ directory: z.string().max(2000), fileName: z.string().min(1).max(240) }).strict(), { workspaceRequired: true, sideEffecting: true, cachePolicy: "never", terminal: true });
