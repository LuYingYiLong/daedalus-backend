...
function assertKeys(value, allowed) {
    for (const key of Object.keys(value))
        if (!allowed.includes(key))
            throw new Error(`Unknown plugin worker event field: ${key}.`);
}
function assertString(value, name, maxLength) {
    if (typeof value !== "string" || value.length === 0 || value.length > maxLength)
        throw new Error(`Invalid plugin worker ${name}.`);
}
function assertRuntimeContext(value) {
    ...
    if (!Array.isArray(value.capabilities) || !value.capabilities.every((capability) => ["tools", "skills", "hooks", "mcp"].includes(String(capability)))) throw new Error("Invalid plugin worker capabilities.");
    if (value.p2Capabilities !== undefined && (!Array.isArray(value.p2Capabilities) || value.p2Capabilities.length > 16 || !value.p2Capabilities.every((capability) => typeof capability === "string" && capability.length <= 64))) throw new Error("Invalid plugin worker P2 capabilities.");
}
...
function assertRegistration(value, kind) {
    ...
    if (kind === "context-provider") {
        ...
        if (!Array.isArray(value.scopes) || value.scopes.length === 0 || value.scopes.length > 3 || !value.scopes.every((scope) => ["workspace", "browser", "plugin"].includes(String(scope)))) throw new Error("Invalid plugin context provider scopes.");
        return;
    }
    if (kind === "tool") {
        assertKeys(value, ["name", "title", "description", "inputSchema", "risk", "workflow", "global"]);
        assertString(value.name, "plugin worker tool name", 128);
        assertString(value.title, "plugin worker tool title", 256);
        assertString(value.description, "plugin worker tool description", 4096);
...
export function parseWorkerEvent(line) {
    const value = JSON.parse(line);
    if (!isRecord(value) || typeof value.type !== "string") throw new Error("Invalid plugin worker event.");
    switch (value.type) {
        case "ready":
        ...
        case "register.context-provider":
            ...
            assertRegistration(value.registration, value.type === "register.context-provider" ? "context-provider" : value.type.slice("register.".length));
            break;
        default:
