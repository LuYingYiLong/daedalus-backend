...
test("P2 manifest keeps capability declarations bounded and summarizes all extension families", (): void => {
	const parsed = pluginP2ManifestSchema.safeParse({
		...
		declarations: {
			...
			contextProviders: [{ id: "ctx", title: "Context", description: "Context", scopes: ["plugin"], handler: "ctx" }],
	...
	assert.deepEqual(summary && {
		...
		contextProviders: summary.contextProviders,
	...
	}, { commands: 1, contextProviders: 1, panels: 1, settings: 1, timelineParts: 1, browser: true, languageServices: 1, events: 1 });
});
...
test("P2 RPC requests require plugin identity for browser control and strict panel state", (): void => {
	...
	assert.equal(pluginP2ManifestSchema.safeParse({ apiVersion: 1, capabilities: { panels: 1 }, declarations: { panels: [{ panelId: "bad", title: "Bad", locations: ["side"], view: [{ type: "RawHtml", html: "<script>" }] }] } }).success, false);
