...
test("loading the active provider returns activeModel instead of its separately remembered model", async (): Promise<void> => {
	await withTempAppData(async (): Promise<void> => {
		...
		assert.equal((await loadProviderConfigWithSecret())?.model, "deepseek-v4-flash");
		assert.equal((await loadProviderConfigWithSecret("deepseek"))?.model, "deepseek-v4-flash-vision-exp");
		assert.equal((await getProviderConfigStatus()).model, "deepseek-v4-flash");
		assert.equal((await getProviderModelSelectionStatus()).activeModel.modelId, "deepseek-v4-flash");
	});
});
