...
import { isSummaryPreparationResult } from "../tools/summary-control.js";
...
export function compactToolResultEntries<T>(
	...
	const nextEntries: T[] = entries.map((entry: T, index: number): T => {
		...
		if (isContextControlResult(content) || isTodoControlResult(content) || isSummaryPreparationResult(content)) return entry;
	...
	return {
		...
		totalChars: nextEntries.reduce((total: number, entry: T): number => {
			const content: string = getContent(entry);
			return total + (isContextControlResult(content) || isTodoControlResult(content) || isSummaryPreparationResult(content) ? 0 : content.length);
		}, 0),
		compactedCount
	};
}
...
export function fitToolResultContent(
	...
	if (isContextControlResult(content) || isTodoControlResult(content) || isSummaryPreparationResult(content)) {
		return {
			content,
			chars: 0,
			truncated: false,
